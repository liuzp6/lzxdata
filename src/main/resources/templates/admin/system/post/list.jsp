                ...
                <div class="titlePanel">
                    ...
                    <div class="toolbar">
                        <div class="btn-group">
                            ...
                            <shiro:hasPermission  name="gwgl:insert"><a id="lr-add" class="btn btn-default" onclick="btn_add()"><i class="fa fa-plus"></i>&nbsp;新增</a></shiro:hasPermission>
                            <shiro:hasPermission  name="gwgl:update"><a id="lr-edit" class="btn btn-default" onclick="btn_edit()"><i class="fa fa-pencil-square-o"></i>&nbsp;编辑</a></shiro:hasPermission>
                            <shiro:hasPermission  name="gwgl:del"><a id="lr-delete" class="btn btn-default" onclick="btn_delete()"><i class="fa fa-trash-o"></i>&nbsp;删除</a></shiro:hasPermission>
                            <shiro:hasPermission  name="gwgl:insert"><a id="lr-delete" class="btn btn-default" onclick="btn_addUser()"><i class="fa fa-plus"></i>&nbsp;编辑成员</a></shiro:hasPermission>
                            <shiro:hasPermission  name="gwgl:select"><a id="lr-delete" class="btn btn-default" onclick="btn_lookUser()"><i class="fa fa-eye"></i>&nbsp;查看成员</a></shiro:hasPermission>
                        </div>
                    </div>
                </div>
...
<script>
    ...
    function reloadList(companyId){
        InitialPage();
        GetGrid();
        var _url = "${pageContext.request.contextPath}/admin/system/post/list_o.html?companyId="+companyId;
    ...
    function GetGrid() {
        ...
        var url= "${pageContext.request.contextPath}/admin/system/post/list_o.html?companyId="+$("#companyId").val();
        $gridTable.jqGrid({
            url: url,
            datatype: "json",
            ...
            colModel: [
                ...
                { label: "岗位名称", name: "name", width: 340, align: "left", sortable: false },
                { label: "上级岗位", name: "parentId",  hidden: true},
                { label: "岗位编号", name: "enCode", width: 100, align: "left", hidden: true },
                { label: "所属部门", name: "deptName", width: 200, align: "left", sortable: false,
                ...
                { label: "创建人", name: "username", width: 100, align: "left", sortable: false ,
                    formatter: function (cellvalue, options, rowObject) {
                        if(rowObject.createUser){
                            return rowObject.createUser.username;
                        }else {
                            return "" ;
                        }
                    }
                },
                { label: "创建时间", name: "createTime", width: 100, align: "left", sortable: false }
            ],
            treeGrid: true,
    ...
    function btn_add() {
        ...
        var url="${pageContext.request.contextPath}/admin/system/post/insert_v.html?companyId="+$("#companyId").val()+"&parentId="+id;
        dialogOpen({
            id: "add",
            title: "添加",
            url: url,
    ...
    function btn_edit() {
        var id = $("#gridTable").jqGridRowValue("id");
        if (id) {
            var url="${pageContext.request.contextPath}/admin/system/post/update_v.html?id="+id;
            dialogOpen({
                id: "update",
                title: "修改",
                url: url,
    ...
    function btn_delete() {
        ...
        if (id) {
            layer.confirm('确认删除么?', {icon: 3, title:'提示'}, function(index){
                ...
                $.ajax({
                    url:'${pageContext.request.contextPath}/admin/system/post/delete_o.html',
    ...
    function btn_addUser() {
        var id = $("#gridTable").jqGridRowValue("id");
        if (id) {
            var url="${pageContext.request.contextPath}/admin/system/post/addUser_v.html?id="+id;
            dialogOpen({
                id: "update",
                title: "编辑成员",
                url: url,
            ...
            layer.msg("请选择的记录！",{icon:3,time:2000}, function(){});
        }
    }
    //查看成员
    function btn_lookUser() {
        var id = $("#gridTable").jqGridRowValue("id");
        if (id) {
            var url="${pageContext.request.contextPath}/admin/system/post/lookUser_v.html?id="+id;
            dialogOpen({
                id: "update",
                title: "查看成员",
                url: url,
            ...
            layer.msg("请选择记录！",{icon:3,time:2000}, function(){});
        }
    }
</script>
</body>
