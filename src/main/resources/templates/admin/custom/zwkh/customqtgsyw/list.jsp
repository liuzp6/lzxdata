            ...
            <table>
                <tr>
                    ...
                    <td style="padding-left: 2px;">
                        <input id="ywmc" type="text" class="form-control"  style="width: 200px;" />
                    </td>
                    <td>&nbsp;运营商：</td>
                    <td style="padding-left: 2px;">
                        <input id="yys" type="text" class="form-control"  style="width: 200px;" />
                    </td>
...
<script>
    ...
    function GetGrid() {
        ...
        $gridTable.jqGrid({
            url: '${pageContext.request.contextPath}/admin/custom/customobusiness/list_o.html?type=custom&customId=${customId}',
            datatype: "json",
            ...
            colModel: [
                { label: '主键', name: 'obsId', hidden: true },
                { label: '业务名称', name: 'ywmc', index: 'lxrName', width: 150, align: 'left' },
                { label: '资费', name: 'zf', index: 'zf', width: 150, align: 'left' },
                { label: '数量', name: 'sl', index: 'sl', width: 150, align: 'left' },
                { label: '运营商', name: 'yys', index: 'yys', width: 150, align: 'left' },
            ],
            viewrecords: true,
            rowNum: 15,
            rowList: [15, 30, 100],
        ...
        function serach(){
            var url = "${pageContext.request.contextPath}/admin/custom/customobusiness/list_o.html?customId=${customId}"
            var queryJson = {
                ywmc: $("#ywmc").val(),
                yys: $("#yys").val()
            }
    ...
    function btn_add() {
    	...
    	var url="${pageContext.request.contextPath}/admin/custom/customobusiness/insert_v.html?customId=${customId}";
        dialogOpen({
            id: "add",
            title: "新增公司业务名称",
            url: url,
    ...
    function btn_edit() {
        ...
        if (id) {
            var url='${pageContext.request.contextPath}/admin/custom/customobusiness/update_v.html?type=system&obsId='+id;
            dialogOpen({
                id: "add",
                title: "修改用户",
                url: url,
    ...
    function btn_detail() {
        ...
        if (id) {
            var url='${pageContext.request.contextPath}/admin/custom/customobusiness/detail_v.html?type=system&obsId='+id;
            dialogOpen({
                id: "add",
                title: "修改用户",
                url: url,
    ...
    function btn_delete(ids) {
        ...
        if (ids) {
	        layer.confirm('确认删除么?', {icon: 7, title:'提示'}, function(index){
	    		$.ajax({
	    			url:'${pageContext.request.contextPath}/admin/custom/customobusiness/del_o.html',
	    			data:'obsIds='+ids+"&time="+new Date(),
	    			type:'post',
	    			dataType: "json",
