            ...
            <table>
                <tr>
                    ...
                    <td style="padding-left: 2px;">
                        <input id="khName" type="text" class="form-control"  style="width: 200px;" />
                    </td>
                    <td>&nbsp;集团编码：</td>
                    <td style="padding-left: 2px;">
                        <input id="jtbm" type="text" class="form-control"  style="width: 200px;" />
                    </td>
                    <td>&nbsp;收入归集编码：</td>
                    <td style="padding-left: 2px;">
                        <input id="srgjbm" type="text" class="form-control"  style="width: 200px;" />
                    </td>
...
<script>
    ...
    function GetGrid() {
        ...
        $gridTable.jqGrid({
            url: '${pageContext.request.contextPath}/admin/custom/customincome/list_o.html?type=custom&customId=${customId}',
            datatype: "json",
            ...
            colModel: [
                { label: '主键', name: 'khsrId', hidden: true },
                { label: '集团编码', name: 'jtbm', index: 'jtbm', width: 100, align: 'left' },
                { label: '收入归集编码', name: 'srgjbm', index: 'srgjbm', width: 100, align: 'left' },
                { label: '客户名称', name: 'khName', index: 'khName', width: 100, align: 'left' },
                { label: '1月份收入(万元)', name: 'one', index: 'one', width: 100, align: 'left' },
                { label: '2月份收入(万元)', name: 'two', index: 'two', width: 100, align: 'left' },
                { label: '3月份收入(万元)', name: 'three', index: 'three', width: 100, align: 'left' },
                { label: '4月份收入(万元)', name: 'four', index: 'four', width: 100, align: 'left' },
                { label: '5月份收入(万元)', name: 'five', index: 'five', width: 100, align: 'left' },
                { label: '6月份收入(万元)', name: 'six', index: 'six', width: 100, align: 'left' },
                { label: '7月份收入(万元)', name: 'seven', index: 'seven', width: 100, align: 'left' },
                { label: '8月份收入(万元)', name: 'eight', index: 'eight', width: 100, align: 'left' },
                { label: '9月份收入(万元)', name: 'nine', index: 'nine', width: 100, align: 'left' },
                { label: '10月份收入(万元)', name: 'ten', index: 'ten', width: 120, align: 'left' },
                { label: '11月份收入(万元)', name: 'eleven', index: 'eleven', width: 120, align: 'left' },
                { label: '12月份收入(万元)', name: 'twelve', index: 'twelve', width: 120, align: 'left' }
            ],
            viewrecords: true,
            rowNum: 15,
            rowList: [15, 30, 100],
        ...
        function serach(){
            var url = "${pageContext.request.contextPath}/admin/custom/customincome/list_o.html?customId=${customId}"
            var queryJson = {
                khName: $("#khName").val(),
                jtbm: $("#jtbm").val(),
                srgjbm: $("#srgjbm").val()
    ...
    function btn_add() {
    	...
    	var url="${pageContext.request.contextPath}/admin/custom/customincome/insert_v.html?customId=${customId}";
        dialogOpen({
            id: "add",
            title: "新增客户信息",
            url: url,
    ...
    function btn_edit() {
        ...
        if (id) {
            var url='${pageContext.request.contextPath}/admin/custom/customincome/update_v.html?type=system&khsrId='+id;
            dialogOpen({
                id: "add",
                title: "修改用户",
                url: url,
    ...
    function btn_detail() {
        ...
        if (id) {
            var url='${pageContext.request.contextPath}/admin/custom/customincome/detail_v.html?type=system&khsrId='+id;
            dialogOpen({
                id: "add",
                title: "修改用户",
                url: url,
    ...
    function btn_delete(ids) {
        ...
        if (ids) {
	        layer.confirm('确认删除么?', {icon: 7, title:'提示'}, function(index){
	    		$.ajax({
	    			url:'${pageContext.request.contextPath}/admin/custom/customincome/del_o.html',
	    			data:'khsrIds='+ids+"&time="+new Date(),
	    			type:'post',
	    			dataType: "json",
