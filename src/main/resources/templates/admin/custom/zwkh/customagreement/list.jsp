        ...
        <div class="title-search">
            <table>
                <tr>
                    ...
                    <td>&nbsp;客户协议名称：</td>
                    <td style="padding-left: 2px;">
                        <input id="khxymc" type="text" class="form-control"  style="width: 200px;" />
                    </td>
...
<script>
    ...
    function GetGrid() {
        ...
        $gridTable.jqGrid({
            url: '${pageContext.request.contextPath}/admin/custom/customagreement/list_o.html?type=custom&customId=${customId}',
            datatype: "json",
            ...
            colModel: [
                { label: '主键', name: 'agreementId', hidden: true },
                { label: '客户名下的协议名称', name: 'khxymc', index: 'khxymc', width: 150, align: 'left' },
                { label: '协议编码', name: 'xybm', index: 'xybm', width: 150, align: 'left' },
                { label: '协议起始日期', name: 'startdtime', index: 'startdtime', width: 150, align: 'left' },
                { label: '协议终止日期', name: 'endtime', index: 'endtime', width: 150, align: 'left' },
                { label: '备注', name: 'beizhu', index: 'beizhu', width: 150, align: 'left' }
            ],
            viewrecords: true,
            rowNum: 15,
            rowList: [15, 30, 100],
        ...
        function serach(){
            var url = "${pageContext.request.contextPath}/admin/custom/customagreement/list_o.html?customId=${customId}"
            var queryJson = {
                khxymc: $("#khxymc").val()
            }
    ...
    function btn_add() {
    	...
    	var url="${pageContext.request.contextPath}/admin/custom/customagreement/insert_v.html?customId=${customId}";
        dialogOpen({
            id: "add",
            title: "新增客户信息",
            url: url,
    ...
    function btn_edit() {
        ...
        if (id) {
            var url='${pageContext.request.contextPath}/admin/custom/customagreement/update_v.html?type=system&agreementId='+id;
            dialogOpen({
                id: "add",
                title: "修改用户",
                url: url,
    ...
    function btn_detail() {
        ...
        if (id) {
            var url='${pageContext.request.contextPath}/admin/custom/customagreement/detail_v.html?type=system&agreementId='+id;
            dialogOpen({
                id: "add",
                title: "修改用户",
                url: url,
    ...
    function btn_delete(ids) {
        ...
        if (ids) {
	        layer.confirm('确认删除么?', {icon: 7, title:'提示'}, function(index){
	    		$.ajax({
	    			url:'${pageContext.request.contextPath}/admin/custom/customagreement/del_o.html',
	    			data:'agreementIds='+ids+"&time="+new Date(),
	    			type:'post',
	    			dataType: "json",
