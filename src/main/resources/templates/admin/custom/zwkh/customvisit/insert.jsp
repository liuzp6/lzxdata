	...
	<div id="form-body" style="overflow: auto;">
		<div style="margin-top: 20px; margin-right: 30px;">
			<table class="form" style="table-layout: auto;">
				<tr>
					<th class="formTitle">联系人:<font>*</font></th>
					<td class="formValue custmerTd" colspan="4">

						<input type="text" class="form-control" name="lxrName" id="lxrName" value="" onblur="Yzkhmc()">
					</td>
					<th class="formTitle">联系电话:<font>*</font></th>
					<td class="formValue custmerTd" colspan="4">

						<input type="text" class="form-control" name="lxdh" id="lxdh" value="" >
					</td>
				</tr>
				<tr>
					<th class="formTitle">拜访内容:<font>*</font></th>
					<td class="formValue custmerTd" colspan="4">

						<input type="text" class="form-control" name="bfnr" id="bfnr" value="" >
					</td>
					<th class="formTitle">拜访方式:<font>*</font></th>
					<td class="formValue custmerTd" colspan="4">

						<input type="text" class="form-control" name="bffs" id="bffs" value="" >
					</td>
				</tr>
				<tr>
					<th class="formTitle">拜访时间:<font>*</font></th>
...
<script>
    ...
    function AcceptClick(sendStatus) {
        ...
        if (!lxrName) {
            top.dialogTop('请填写联系人', 'error');
            return false;
        }
        if($("#lxdh").val() == ""){
            top.dialogTop('请填写联系电话', 'error');
            return false;
		}
        if($("#bfnr").val() == ""){
            top.dialogTop('请填写拜访内容', 'error');
            return false;
        }
        if($("#bffs").val() == ""){
            top.dialogTop('请填写拜访方式', 'error');
            return false;
        }
        if($("#bfsj").val() == ""){
            top.dialogTop('请填写拜访时间', 'error');
            return false;
        }
        <%--var flag = false;--%>
        <%--$.ajax({--%>
            <%--url: "${pageContext.request.contextPath}/admin/custom/customvisit/khmcajax_v.html",--%>
            <%--type: 'POST',--%>
            <%--async: false,--%>
            <%--dataType:"json",--%>
        ...
        <%--}--%>
        $.SaveForm({
            url: "${pageContext.request.contextPath}/admin/custom/customvisit/insert_o.html?customId=${customId}",
