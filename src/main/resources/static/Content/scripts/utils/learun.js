window.learun = {}, function (n, t) {
	...
	n.extend(t, {
		...
		request : function (n) {
			for (var u = location.search.slice(1), r = u.split("&"), i, t = 0; t < r.length; t++)
				if (i = r[t].split("="), i[0] == n)
					return unescape(i[1]) == "undefined" ? "" : unescape(i[1]);
			return ""
		},
		...
		createGuid : function () {
			for (var t = "", i, n = 1; n <= 32; n++)
				i = Math.floor(Math.random() * 16).toString(16), t += i, (n == 8 || n == 12 || n == 16 || n == 20) && (t += "-");
			return t
		},
		...
		toDecimal : function (n) {
			...
			i,
			t;
			for (n == null && (n = "0"), n = n.toString().replace(/\$|\,/g, ""), isNaN(n) && (n = "0"), r = n == (n = Math.abs(n)), n = Math.floor(n * 100 + .50000000001), i = n % 100, n = Math.floor(n / 100).toString(), i < 10 && (i = "0" + i), t = 0; t < Math.floor((n.length - (1 + t)) / 3); t++)
		...
		loadSystemForm : function (i, r) {
			...
			f = function () {
				var n = t.getIframe(i);
				!n.$ || n.$("body").find("[data-systemHideField]").hide();
				t.loading({
					isShow : !1
				})
			};
		...
		saveSystemFormData : function (n, i) {
			var r = t.getIframe(n);
			!r.$ || !r.AcceptClick || r.AcceptClick(i)
		},
		setSystemFormFieldsAuthrize : function (n, i) {
			var r = t.getIframe(n);
			!r.$ || !r.setSystemFieldsAuthorize || r.setSystemFieldsAuthorize(i)
		},
...
(window.jQuery, window.learun), function (n, t) {
	"use strict";
		...
		return e ? (i = n.extend({
				...
				loadData : function () {
					if (!i.url) {
						...
						i.data = t.ajax.asyncGet({
								url : i.url,
								data : i.param,
								type : i.method
							}), !i.dataName || (i.data = i.data[i.dataName])
				}
			}, f.loadData(), r = f.rendering(), f.renderingData(r, i), i.allowSearch && (r.find(".ui-select-option-search").find("input").bind("keypress", function () {
					if (event.keyCode == "13") {
						var t = n(this);
					...
					n(this).select()
				})[0].opt = i), u.unbind("click"), u.bind("click", function (t) {
				var s;
				if (u.attr("readonly") == "readonly" || u.attr("disabled") == "disabled")
					return !1;
				if (n(this).addClass("ui-select-focus"), r.is(":hidden")) {
					...
					r.attr("data-show") ? r.hide() : r.slideUp(150);
				t.stopPropagation()
	...
	n.fn.comboBoxSetValue = function (i) {
		...
		if (!t.isNullOrEmpty(i))
			return r = n(this), f = n("#" + r.attr("id") + "-option"), r.attr("data-value", i), u = f.find("ul").find("[data-value=" + i + "]").html(), u && (r.attr("data-text", u), r.find(".ui-select-text").html(u).css("color", "#000"), f.find("ul").find("[data-value=" + i + "]").addClass("liactive")), r.trigger("change"), r
	};
	n.fn.comboBoxTree = function (i) {
		...
		f = {
			...
			loadData : function (i) {
				...
				i.dataItemName ? (i.data = [], n.each(r, function (n, t) {
		...
		i.allowSearch && (r.find(".ui-select-option-search").find("input").bind("keypress", function () {
				if (event.keyCode == "13") {
					var t = n(this),
					i = n(this).val(),
		...
		u.find(".ui-select-text").bind("click", function (t) {
			var s;
			if (u.attr("readonly") == "readonly" || u.attr("disabled") == "disabled")
				return !1;
	...
	n.fn.conTextMenu = function () {
		...
		for (i.hide(), t = 0; t < r.length; t++)
			r[t].getElementsByTagName("ul")[0] && (r[t].className = "sub"), r[t].onmouseover = function () {
				var i = this,
				n = i.getElementsByTagName("ul");
				i.className += " active";
				n[0] && (clearTimeout(o), f = setTimeout(function () {
							for (t = 0; t < i.parentNode.children.length; t++)
								i.parentNode.children[t].getElementsByTagName("ul")[0] && (i.parentNode.children[t].getElementsByTagName("ul")[0].style.display = "none");
		...
		r[t].onmouseout = function () {
			...
			i = setTimeout(function () {
					for (t = 0; t < n.parentNode.children.length; t++)
						n.parentNode.children[t].getElementsByTagName("ul")[0] && (n.parentNode.children[t].getElementsByTagName("ul")[0].style.display = "none")
				}, 300)
		};
		n(e).bind("contextmenu", function () {
	...
	n.fn.authorizeButton = function () {
		...
		r != undefined && n.each(r, function (n) {
	...
	n.fn.authorizeColModel = function () {
		...
		t != undefined && n.each(t, function (n) {
	...
	n.fn.jqGridRowValue = function (t) {
		...
		i;
		if (u != undefined && u != "")
			for (o = u.length, f = 0; f < o; f++)
				i = r.jqGrid("getRowData", u[f]), e.push(i[t]);
		else
			i = r.jqGrid("getRowData", r.jqGrid("getGridParam", "selrow")), e.push(i[t]);
		return String(e)
	};
	n.fn.jqGridRow = function () {
		...
		t;
		if (f != "")
			for (e = f.length, r = 0; r < e; r++)
				t = i.jqGrid("getRowData", f[r]), u.push(t);
		else
			t = i.jqGrid("getRowData", i.jqGrid("getGridParam", "selrow")), u.push(t);
		return u
	};
	n.fn.uploadifyEx = function (i) {
		...
		if (i = n.extend({
			...
			u.removeAttr("id"), u.html('<input id="' + r + '" type="file" />'), u = n("#" + r), i.fileTypeExts = i.fileTypeExts == "" ? "*.avi;*.mp3;*.mp4;*.bmp;*.ico;*.gif;*.jpeg;*.jpg;*.png;*.psd; *.rar;*.zip;*.swf;*.log;*.pdf;*.doc;*.docx;*.ppt;*.pptx;*.txt; *.xls; *.xlsx;" : "*." + i.fileTypeExts.replace(/,/g, ";*.") + ";", u.uploadify({
				method : "post",
				uploader : i.url,
				...
				onSelect : function (u) {
					...
					f.find(".cancel").on("click", function () {
				...
				onUploadSuccess : function (t, u) {
					var e,
					f;
					if (n("#" + t.id).find(".uploadify-progress").remove(), n("#" + t.id).find(".data").html(" 恭喜您，上传成功！"), n("#" + t.id).prepend('<a class="succeed" title="成功"><i class="fa fa-check-circle"><\/i><\/a>'), e = JSON.parse(u), n("#" + t.id).attr("data-fileId", e.fileId), f = n("#" + r).attr("data-value"), f != undefined && f != "" && f != "undefined" ? f += "," : f = "", n("#" + r).attr("data-value", f + e.fileId), i.onUploadSuccess)
						i.onUploadSuccess(e)
				},
			...
			}), n("#" + r + "-button").prepend('<i style="opacity: 0.6;" class="fa fa-cloud-upload"><\/i>&nbsp;'), n("#" + r + "-queue").hide(), n("#" + r).attr("type", "uploadify");
		else {
			...
			f.on("startUpload", function () {
				var n = u.find(".uploadify-queue");
				n.show()
			});
			f.on("uploadStart", function (r) {
			...
			f.on("uploadComplete", function (i) {
	...
	n.fn.uploadifyExSet = function (i, r) {
		...
		t.setForm({
			...
			success : function (t) {
				n.each(t, function (n, t) {
					f.show();
					var i = '<div id="' + t.F_Id + '"  class="uploadify-queue-item olduploadify-queue-item" ><a class="succeed" title="成功"><i class="fa fa-check-circle"><\/i><\/a><div style="float:left;width:50px;margin-right:2px;"><img src="/Content/images/filetype/' + t.F_FileType + '.png" style="width:40px;height:40px;"><\/div>';
					(r == undefined || r.isRemove) && (i += '<div class="cancel remove" data-fileId="' + t.F_Id + '"><a title="删除" style="display: none;"><i class="fa fa-trash-o "><\/i><\/a><\/div>');
					(r == undefined || r.isDown) && (i += '<div class="cancel down" data-fileId="' + t.F_Id + '"><a title="下载" style="display: none;margin-right:10px;"><i class="fa fa-download"><\/i><\/a><\/div>');
		...
		f.find(".olduploadify-queue-item").find(".remove").on("click", function () {
		...
		f.find(".olduploadify-queue-item").find(".down").on("click", function () {
			var i = n(this).attr("data-fileId");
			t.downFile({
...
(window.jQuery, window.learun), function (n) {
